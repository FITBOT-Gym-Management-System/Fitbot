...
function getTodayTasks(passValue){
    let date = new Date();
    ...
    $.ajax({
        ...
        if(passValue==1) {
            let countName = 0;
            // alert("in first "+countName);

           /* $.ajax({
                method:'POST',
                url:"maintainerDailyTasksCheck",
                data:{currentDate:currentDate},
                // dataType: 'json',
                // contentType: "application/json",
            }).done(function (result){
            ...
            $.map(result, function (x) {
                // console.log(x.equipment_has_modified);
                ...
                // alert("in second "+countName);
                //&& (equipment_has_modified.toString() == "0")

                // console.log(currentDate);
                // console.log(dateCompare);
                // console.log(currentDate1);
                if ((currentDate == dateCompare) || (currentDate1 == dateCompare)) {
                    // alert(countName);
                    if (countName < 2) {
                        $.ajax({
                            method: 'POST',
